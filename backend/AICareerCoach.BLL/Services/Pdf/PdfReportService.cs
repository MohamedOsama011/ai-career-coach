...
namespace AICareerCoach.BLL.Services.Pdf
{
    public class PdfReportService : IPdfReportService
    {
        ...
        public byte[] GenerateModifiedCvReport(string modifiedText)
        {
            var normalized = NormalizeCvText(modifiedText ?? "");
            return Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Column(h =>
                    {
                        h.Item().Row(r =>
                        {
                            r.RelativeItem().Text("CV").Bold().FontSize(14).FontColor(Primary);
                            r.ConstantItem(120).AlignRight().Text(DateTime.Now.ToString("MMMM dd, yyyy")).FontSize(10).FontColor(TextMuted);
                        });
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        var sectionPattern = @"\b(Professional Summary|Core Competencies|Technical Skills?|Technical Expertise|Professional Experience|Work History|Additional Skills|Certifications?(?:\s*[&]\s*Training)?|Education|Experience|Projects?|Achievements|Languages?|Interests|Publications|Summary|Profile|Objective|Training|References|Internship|Research)\b";
                        ...
                        foreach (var line in lines)
                        {
                            ...
                            if (System.Text.RegularExpressions.Regex.IsMatch(trimmed, sectionPattern, System.Text.RegularExpressions.RegexOptions.IgnoreCase))
                            {
                                col.Item().PaddingTop(12).Text(trimmed.ToUpperInvariant())
                                    .FontSize(11).Bold().FontColor(Primary);
                                col.Item().PaddingTop(2).Height(1).Background(PrimaryMid);
                            }
                            else
                            {
                                col.Item().Text(trimmed).FontSize(10).FontColor("#374151");
                            }
                        }
                    });
        ...
        {
            var r = text.Replace("\r\n", "\n").Replace("\r", "\n");
            r = System.Text.RegularExpressions.Regex.Replace(r, @"([a-z])([A-Z][a-z])", "$1 $2");
            r = System.Text.RegularExpressions.Regex.Replace(r, @"([a-z])([A-Z])", "$1 $2");
            r = System.Text.RegularExpressions.Regex.Replace(r, @"(\w\.)([A-Z])", "$1 $2");

            r = System.Text.RegularExpressions.Regex.Replace(r,
                @"\b(PROFESSIONAL (?:SUMMARY|EXPERIENCE)|SUMMARY|PROFILE|OBJECTIVE|EDUCATION|EXPERIENCE|SKILLS|PROJECTS|CERTIFICATIONS|TECHNICAL SKILLS|WORK HISTORY|ACHIEVEMENTS|LANGUAGES|INTERESTS|PUBLICATIONS|TRAINING|REFERENCES|INTERNSHIP|RESEARCH)\b",
                "\n$1\n");
            ...
            r = System.Text.RegularExpressions.Regex.Replace(r, @"\n +", "\n");
            r = System.Text.RegularExpressions.Regex.Replace(r, @" {3,}", " ");
            r = System.Text.RegularExpressions.Regex.Replace(r, @"\n{3,}", "\n\n");
            return r.Trim();
        }
