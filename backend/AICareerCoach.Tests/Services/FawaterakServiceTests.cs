...
namespace AICareerCoach.Tests.Services
{
    public class FawaterakServiceTests
    {
        ...
        private static FawaterakService CreateService(HttpClient http, IConfiguration config, AICareerCoachDbContext context, Mock<IFawaterakTokenService>? tokenService = null, Mock<IUserSubscriptionService>? userSubService = null)
        {
            tokenService ??= new Mock<IFawaterakTokenService>();
            userSubService ??= new Mock<IUserSubscriptionService>();
            var logger = new Mock<ILogger<FawaterakService>>();
            return new FawaterakService(http, config, context, tokenService.Object, userSubService.Object, logger.Object);
        }
        ...
        public async Task CreatePaymentAsync_ReturnsError_WhenPlanNotFound()
        {
            ...
            var tokenService = new Mock<IFawaterakTokenService>();
            var userSubService = new Mock<IUserSubscriptionService>();
        ...
        public async Task CreatePaymentAsync_Success_ReturnsPaymentMethods()
        {
            ...
            var methodsDto = new GetPaymentMethodsResponseDto
            {
                Status = "success",
                Data = new List<PaymentMethodDto>
                {
                    new PaymentMethodDto { PaymentMethodId = 1, Name = "pm", NameEn = "pm", NameAr = "", Redirect = "", Logo = "" }
                }
            };
            ...
            var pmList = Assert.IsType<List<PaymentMethodDto>>(res.Data);
            Assert.Single(pmList);
            Assert.Equal(1, pmList[0].PaymentMethodId);
            Assert.Equal("pm", pmList[0].Name);
            Assert.Equal("pm", pmList[0].NameEn);
        }
        ...
        public async Task GetPaymentMethodsAsync_Throws_OnHttpError()
        {
            ...
            var svc = CreateService(http, config, context, tokenService, new Mock<IUserSubscriptionService>());
        ...
        public async Task ExecutePaymentAsync_Success_UpdatesPaymentIntentKey()
        {
            ...
            var responseDto = new ExecutePaymentResponseDto
            {
                Status = "success",
                Data = new FawaterakPaymentDataDto { IntentKey = "intent-123" }
            };
        ...
        public async Task HandleSuccessWebhookAsync_InvalidHash_ReturnsFalse()
        {
            // Arrange
            using var context = CreateInMemoryContext();
            var dto = new WebhookSuccessDto { TransactionId = 1, TransactionKey = "tk", PaymentMethod = "1", Status = "s", PaidAt = DateTime.UtcNow, PaidAmount = 0, PaidCurrency = "EGP" };
        ...
        public async Task HandleSuccessWebhookAsync_ProcessesPayment_Succeeds()
        {
            ...
            var dto = new WebhookSuccessDto { TransactionId = 777, TransactionKey = "ikey-77", PaymentMethod = "2", Status = "paid" };
            // set PayLoad with order_id matching UserSubscription.Id for cancel-format matching
            var payloadJson = JsonSerializer.Serialize(new { order_id = us.Id.ToString() });
            dto.PayLoad = JsonSerializer.Deserialize<JsonElement>(payloadJson);
            // compute valid hash
            var secret = "mysecret";
